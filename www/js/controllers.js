...
.controller('AppCtrl', ['Chrono.Api', '$log', '$scope', '$window', 'UserService', '$state', '$timeout', '$interval', '$log', '$ionicPopup', '$http', '$ionicPopover', '$ionicLoading', '$ionicModal','$cordovaBarcodeScanner', function(api, $log, $scope, $window, UserService, $state, $timeout, $interval, $log, $ionicPopup, $http, $ionicPopover, $ionicLoading, $ionicModal,$cordovaBarcodeScanner) {
	$scope.game = {
		isLoaded: false,
		isPaused: false,
		player: null,
	...
	$scope.init = function() {
		...
		.then(function() {
			...
			if ($scope.firstIn == 0) {
				...
				$scope.handleEvent();
					...
					if (exception instanceof api.exceptions.TokenNotValidException) {
		...
		$scope.killPopup = function() {
			...
			$ionicPopup.show({
				template: '<div class="item item-input-inset"><label class="item-input-wrapper"><input type="text" ng-model="trans.id" placeholder="裝置 ID">  </label> <button class="button button-small" ng-click="scanQr()">Scan QR</button> </div>',
				title: '輸入擊殺目標',
				scope: $scope,
				buttons: [
				{ text: '取消' },
