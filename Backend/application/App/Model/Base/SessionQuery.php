...
/**
 * Base class that represents a query for the 'chrono_session' table.
 *
 *
 *
 * @method     ChildSessionQuery orderByToken($order = Criteria::ASC) Order by the token column
 * @method     ChildSessionQuery orderByPlayerId($order = Criteria::ASC) Order by the player_id column
 ...
 * @method     ChildSessionQuery groupByPlayerId() Group by the player_id column
 ...
 *
 * @method     ChildSessionQuery leftJoinPlayer($relationAlias = null) Adds a LEFT JOIN clause to the query using the Player relation
 * @method     ChildSessionQuery rightJoinPlayer($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Player relation
 * @method     ChildSessionQuery innerJoinPlayer($relationAlias = null) Adds a INNER JOIN clause to the query using the Player relation
 *
 * @method     ChildSessionQuery joinWithPlayer($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Player relation
 *
 * @method     ChildSessionQuery leftJoinWithPlayer() Adds a LEFT JOIN clause and with to the query using the Player relation
 * @method     ChildSessionQuery rightJoinWithPlayer() Adds a RIGHT JOIN clause and with to the query using the Player relation
 * @method     ChildSessionQuery innerJoinWithPlayer() Adds a INNER JOIN clause and with to the query using the Player relation
 *
 ...
 * @method     ChildSession findOneByPlayerId(int $player_id) Return the first ChildSession filtered by the player_id column
 ...
 * @method     ChildSession requireOneByPlayerId(int $player_id) Return the first ChildSession filtered by the player_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildSession[]|ObjectCollection findByPlayerId(int $player_id) Return ChildSession objects filtered by the player_id column
...
abstract class SessionQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'chrono', $modelName = '\\App\\Model\\Session', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT token, player_id, expired_at, created_at, updated_at FROM chrono_session WHERE token = :p0';
        try {
    ...
    public function filterByPlayerId($playerId = null, $comparison = null)
    {
        if (is_array($playerId)) {
            ...
            if (isset($playerId['min'])) {
                $this->addUsingAlias(SessionTableMap::COL_PLAYER_ID, $playerId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($playerId['max'])) {
                $this->addUsingAlias(SessionTableMap::COL_PLAYER_ID, $playerId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(SessionTableMap::COL_PLAYER_ID, $playerId, $comparison);
    }
    ...
    public function filterByPlayer($player, $comparison = null)
    {
        if ($player instanceof \App\Model\Player) {
            return $this
                ->addUsingAlias(SessionTableMap::COL_PLAYER_ID, $player->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(SessionTableMap::COL_PLAYER_ID, $player->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
    ...
    /**
     * Deletes all rows from the chrono_session table.
     *
