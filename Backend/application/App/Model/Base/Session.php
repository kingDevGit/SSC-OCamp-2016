...
/**
 * Base class that represents a row from the 'chrono_session' table.
 *
 *
 *
...
abstract class Session implements ActiveRecordInterface
{
    ...
    public function setPlayerId($v)
    {
        ...
        if ($this->player_id !== $v) {
            $this->player_id = $v;
            $this->modifiedColumns[SessionTableMap::COL_PLAYER_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SessionTableMap::translateFieldName('PlayerId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->player_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SessionTableMap::translateFieldName('ExpiredAt', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : SessionTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SessionTableMap::COL_PLAYER_ID)) {
        ...
        $sql = sprintf(
            'INSERT INTO chrono_session (%s) VALUES (%s)',
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SessionTableMap::COL_PLAYER_ID)) {
            $criteria->add(SessionTableMap::COL_PLAYER_ID, $this->player_id);
        }
        if ($this->isColumnModified(SessionTableMap::COL_EXPIRED_AT)) {
    ...
    {
        if (null !== $this->aPlayer) {
            $this->aPlayer->removeSession($this);
        }
        $this->token = null;
        $this->player_id = null;
