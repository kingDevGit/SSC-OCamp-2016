...
/**
 * Base class that represents a row from the 'chrono_notification' table.
 *
 *
 *
...
abstract class Notification implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\Model\\Map\\NotificationTableMap';
    ...
    /**
     * The value for the to_player field.
     *
     * @var        int
     */
    protected $to_player;
    ...
    /**
     * Initializes internal state of App\Model\Base\Notification object.
     */
    ...
    /**
     * Get the [to_player] column value.
     *
     * @return int
     */
     ...
     * @return $this|\App\Model\Notification The current object (for fluent API support)
     */
    ...
    /**
     * Set the value of [to_player] column.
     *
     * @param int $v new value
     * @return $this|\App\Model\Notification The current object (for fluent API support)
     */
    public function setToPlayer($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }
        ...
        if ($this->to_player !== $v) {
            $this->to_player = $v;
            $this->modifiedColumns[NotificationTableMap::COL_TO_PLAYER] = true;
        }
    ...
    public function setMessage($v)
    {
        ...
        if ($this->message !== $v) {
            $this->message = $v;
            $this->modifiedColumns[NotificationTableMap::COL_MESSAGE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : NotificationTableMap::translateFieldName('ToPlayer', TableMap::TYPE_PHPNAME, $indexType)];
            $this->to_player = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : NotificationTableMap::translateFieldName('Message', TableMap::TYPE_PHPNAME, $indexType)];
            $this->message = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : NotificationTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : NotificationTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\App\\Model\\Notification'), 0, $e);
        }
    }
    ...
    {
        if ($this->aPlayer !== null && $this->to_player !== $this->aPlayer->getId()) {
            $this->aPlayer = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(NotificationTableMap::COL_TO_PLAYER)) {
            $modifiedColumns[':p' . $index++]  = 'to_player';
        }
        if ($this->isColumnModified(NotificationTableMap::COL_MESSAGE)) {
        ...
        $sql = sprintf(
            'INSERT INTO chrono_notification (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id':
                        ...
                        break;
                    case 'to_player':
                        $stmt->bindValue($identifier, $this->to_player, PDO::PARAM_INT);
                        break;
                    case 'message':
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\Model\Notification The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(NotificationTableMap::COL_TO_PLAYER)) {
            $criteria->add(NotificationTableMap::COL_TO_PLAYER, $this->to_player);
        }
        if ($this->isColumnModified(NotificationTableMap::COL_MESSAGE)) {
            $criteria->add(NotificationTableMap::COL_MESSAGE, $this->message);
        }
        if ($this->isColumnModified(NotificationTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\Model\Notification (or compatible) type.
    ...
    {
        $copyObj->setToPlayer($this->getToPlayer());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\Model\Notification Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ($this->aPlayer === null && ($this->to_player !== null)) {
            $this->aPlayer = ChildPlayerQuery::create()->findPk($this->to_player, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aPlayer->addNotifications($this);
             */
        }
    ...
    {
        if (null !== $this->aPlayer) {
            $this->aPlayer->removeNotification($this);
        }
        $this->id = null;
        $this->to_player = null;
        $this->message = null;
