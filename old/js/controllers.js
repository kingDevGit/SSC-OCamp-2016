...
.controller('AppCtrl', ['Chrono.Api', '$log', '$scope', '$window','UserService','$state','$timeout','$interval','$log','$ionicPopup','$http','$ionicPopover','$ionicLoading','$ionicPlatform','$cordovaBarcodeScanner', function(api,$log, $scope, $window,UserService,$state,$timeout,$interval,$log,$ionicPopup,$http,$ionicPopover,$ionicLoading,$ionicPlatform,$cordovaBarcodeScanner) {
...
$scope.trans.id=result.text;
	...
	$scope.allowTrans=function(){
		if($scope.game.settings.allow_transaction=="no" && $scope.game.player.tags.indexOf("oc")== -1){
			return true;
		}else{
			return false;
		}
	...
	$scope.tranTimePopup = function() {
		$scope.data = {};

		var myPopup = $ionicPopup.show({
			template: '<div class="item item-input-inset"><label class="item-input-wrapper"><input type="text" ng-model="trans.id" placeholder="裝置 ID">  </label> <button class="button button-small" ng-click="scanQr()">Scan QR</button> </div><br ><input type="text" placeholder="時間 (分鐘)" ng-model="trans.time">',
			title: '輸入目標 ID 和時間 ',
			subTitle: '時間以分鐘計算',
			scope: $scope,
			buttons: [
	...
	$scope.tranTime=function(trans){

		$log.debug("Giving "+trans.id+" "+trans.time+" mins");
		$http.post($scope.host + '/transaction',
			
		{
			'address':trans.id,
			'second':trans.time*60
		}
		,{
			headers: { 'Chrono-Token': UserService.getUser() , 'Content-Type': 'application/json; charset=UTF-8'}
		})
		.then(function(response) {
			$log.debug('Success' + response);
		})
		.catch(function(exception) {
			$log.debug('Fail' + JSON.stringify(exception));
			throw exception;
		});
